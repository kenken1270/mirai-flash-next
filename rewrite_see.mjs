...
const SEE_STAMPS = [
  ...
  { score: 4, emoji: '😊', label: 'よくできた', color: 'bg-yellow-100 border-yellow-400' },
  { score: 3, emoji: '😐', label: 'まあまあ', color: 'bg-green-100 border-green-400' },
  { score: 2, emoji: '😓', label: 'むずかしかった', color: 'bg-blue-100 border-blue-400' },
  { score: 1, emoji: '😭', label: 'ぜんぜんダメ', color: 'bg-purple-100 border-purple-400' },
]

const META_QUESTIONS = [
  { id: 'good', label: '✅ うまくできたことは？', placeholder: 'たとえば「漢字を3つ覚えられた」' },
  { id: 'hard', label: '🤔 むずかしかったことは？', placeholder: 'たとえば「計算ミスが多かった」' },
  { id: 'next', label: '🚀 次はどうする？', placeholder: 'たとえば「計算問題を5問練習する」' },
]

const NEXT_ACTIONS = [
  { id: 'retry',     label: 'もう一度やる',   emoji: '🔁' },
  { id: 'help',      label: '先生に聞く',     emoji: '🙋' },
  { id: 'next_task', label: '次のタスクへ',   emoji: '➡️' },
  { id: 'rest',      label: 'ひと休みする',   emoji: '☕' },
]
...
function SeeContent() {
  ...
  const handleSave = async () => {
    if (!task || !user || stamp === null || nextAction === null) return
    setSaving(true)
    try {
      ...
      const newPoints = (user.current_points ?? 0) + addExp
      await supabase.from('users').update({ current_points: newPoints }).eq('username', user.username)
      ...
      <div className="max-w-lg mx-auto pt-6">
        ...
        {step === 2 && (
          <div className="bg-white rounded-2xl shadow p-6 space-y-4">
            ...
            <p className="text-sm text-gray-400">3つの問いに答えてね（スキップもOK）</p>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={() => setStep(3)} className="flex-1 py-3 bg-indigo-600 text-white rounded-xl font-bold">次 へ →</button>
            </div>
          </div>
        )}

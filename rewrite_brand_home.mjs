...
import { loadUser, loadTodayTasks } from '@/lib/student'
...
function getMascot(tasks: PlanRow[], done: number) {
  if (tasks.length === 0) return { emoji: '🐕', msg: 'きょうはおやすみ？のんびりしよ〜', face: 'zzz' }
  if (done === tasks.length) return { emoji: '🐕', msg: 'ぜんぶできた！！すごすぎる！！', face: 'excited' }
  if (done === 0) return { emoji: '🐕', msg: 'さあはじめよう！いっしょにがんばる！', face: 'normal' }
  return { emoji: '🐕', msg: \`もう\${done}こできた！あとちょっと！\`, face: 'happy' }
}
...
export default function StudentHome() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (u) setUser(u)
      const t = await loadTodayTasks(session.user.id)
      if (t) {
        setTasks(t)
        setDoneTasks(t.filter((x: PlanRow) => x.is_done).length)
      }
        ...
        <div className="max-w-lg mx-auto">
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <div className="text-right">
              <div className="bg-amber-600 text-white text-xs font-black px-3 py-1 rounded-full">
                Lv.{level}
              </div>
              <p className="text-amber-800 text-xs mt-1">🔥 {user?.streak ?? 0}日連続</p>
            </div>
          </div>
          ...
          <div className="flex justify-between mt-1">
            <p className="text-amber-800 text-xs font-bold">XP {user?.current_points ?? 0}</p>
      ...
      <div className="max-w-lg mx-auto px-4 mt-4 space-y-4">
        ...
        <div className="bg-white rounded-2xl p-4 shadow border border-amber-100">
          ...
          <div className="bg-amber-100 rounded-full h-4 overflow-hidden">
            <div
              className="h-full bg-amber-400 rounded-full transition-all duration-700 flex items-center justify-end pr-2"
              style={{ width: \`\${progressPct}%\` }}
            >
              {progressPct > 20 && <span className="text-xs text-amber-900 font-black">{progressPct}%</span>}
            </div>
          </div>
          {tasks.length > 0 && (
            <button
            ...
            >
              {doneTasks === tasks.length ? '✅ 全部おわった！' : '▶ つづきをやる'}
            </button>
          )}
        </div>

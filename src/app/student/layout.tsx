...
export default function StudentLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[#FFFDF0] flex flex-col font-sans text-gray-800">
      ...
      {isOpen && (
        <>
          <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[55]" onClick={() => setIsOpen(false)}></div>
          <div className="fixed top-0 right-0 bottom-0 w-64 bg-white z-[56] shadow-2xl p-6 pt-20 flex flex-col">
            ...
            <button onClick={() => router.push('/login')} className="py-3 rounded-xl border-2 border-gray-100 text-gray-400 font-bold text-xs">ログアウト</button>
          </div>
        </>
      )}

...
function AnagramContent() {
  ...
  const initQuiz = useCallback((word: string) => {
    const chars = word
      .split('')
      .filter(c => c !== ' ')
      .map((c, i) => ({ id: i, char: c }))
  ...
  useEffect(() => {
    async function fetchCards() {
      setLoading(true)
      const bid = bookId ? Number(bookId) : 0
      const sid = setId ? Number(setId) : 0
      if (!bid && !sid) {
        setLoading(false)
        return
      }

      let query = supabase.from('flashcards_v3').select('*').order('item_no')
      if (bid) {
        const { data: sets } = await supabase.from('flashcard_sets').select('id').eq('book_id', bid)
      ...
      if (questionLimit > 0 && Number.isFinite(questionLimit)) {
        list = shuffle(list).slice(0, Math.min(questionLimit, list.length))
      } else {
        list = shuffle(list)
      }
      setCards(list)
      if (list.length > 0) initQuiz(list[0].lang1)
      setLoading(false)
    }
    void fetchCards()
  }, [setId, bookId, startNo, endNo, questionLimit, initQuiz])
      ...
      <div className="min-h-screen bg-[#FFFDF0] flex flex-col items-center justify-center p-6 text-center">
        <div className="bg-white p-10 rounded-3xl shadow-xl border-4 border-yellow-400 space-y-6 animate-in zoom-in duration-500">
          ...
          <button
            type="button"
            onClick={() => router.back()}
            className="w-full py-4 bg-yellow-400 text-gray-900 rounded-2xl font-bold shadow-md active:scale-95 transition"
          >
            一覧へもどる
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-[#FFFDF0] flex flex-col items-center">
      <div className="w-full bg-yellow-400 p-4 shadow-md flex justify-between items-center text-gray-900 font-bold">
        <button type="button" onClick={() => router.back()} className="bg-white/50 px-3 py-1 rounded-full text-sm">
          ← 戻る
        </button>
        <span>🧩 ならべかえクイズ</span>
      ...
      <div className="flex-1 w-full max-w-md flex flex-col items-center justify-center p-6 gap-8">
          ...
          <div className="flex flex-wrap gap-2 min-h-[4.5rem] items-center justify-center p-4 bg-white/50 rounded-2xl border-2 border-dashed border-yellow-200">
            {answerChars.map(item => (
              <button
                key={item.id}
                type="button"
                onClick={() => handleCharClick(item, true)}
          ...
          <div className="flex flex-wrap gap-3 justify-center">
            {shuffledChars.map(item => (
              <button
                key={item.id}
                type="button"
                onClick={() => handleCharClick(item, false)}
        ...
        <button
          ...
          className="text-gray-400 text-xs font-bold underline decoration-dotted"
        >
          ならべなおす
        </button>
      </div>

      ...
      <div className="max-w-lg mx-auto pt-6 space-y-5">
        ...
        <div className="bg-white rounded-2xl shadow p-5">
          <div className="flex justify-between text-sm mb-2">
            <span className="font-bold text-gray-600">今日の進捗</span>
            <span className="font-bold text-orange-500">{done}/{total} 完了（{pct}%）</span>
          </div>
          ...
          {pct === 100 && total > 0 && (
            <p className="text-center text-sm font-bold text-orange-500 mt-2">🎉 今日のタスク全部完了！すごい！</p>
          )}
        </div>
        ...
        {tasks.length === 0 ? (
          <div className="bg-white rounded-2xl shadow p-8 text-center space-y-3">
            ...
            <button
              ...
              className="mt-2 px-6 py-3 bg-orange-500 text-white rounded-xl font-bold"
            >
              📋 計画ページへ
            </button>
          </div>
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1">
                    ...
                    <div className="flex gap-3 mt-1">
                      <span className="text-xs text-gray-400">⏱ {task.planned_minutes ?? 0}分</span>
                      {task.task_type && (
                        <span className="text-xs bg-orange-50 text-orange-400 px-2 py-0.5 rounded-full">{task.task_type}</span>
                      )}
                      {task.mid_plan && (
                        <span className="text-xs text-gray-300">{task.mid_plan}</span>
                      )}
                    </div>
                  </div>
                  {task.is_done !== 1 ? (
                    ...
                    <button
                      onClick={() => router.push('/student/see?task_id=' + task.id)}
                      className="px-3 py-2 bg-green-100 text-green-600 rounded-xl text-xs font-bold whitespace-nowrap"
                    >
                      📝 振り返る
                    </button>
                  )}
        ...
        <div className="flex gap-3">
          <button
            onClick={() => router.push('/student/plan')}
            className="flex-1 py-3 border border-orange-300 text-orange-600 rounded-2xl font-bold text-sm"
          >
            📋 計画を編集
          </button>
          <button
            onClick={() => router.push('/student/break')}
            className="flex-1 py-3 border border-blue-200 text-blue-400 rounded-2xl font-bold text-sm"
          >
            ☕ 休憩する
          </button>
        </div>
